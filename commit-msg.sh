...
whitelisted_prefixes=(FIX HOTFIX REFACTORING TYPO TECH TEST Merge) # allowed words which used to determine commit kind

# regex to validate commit message
commit_regex="^($(join \| ${whitelisted_prefixes[@]})|($(join \| ${jira_project_prefixes[@]})){1}\-[0-9]+){1}[[:blank:]]"

error_msg="
ABORTING COMMIT:
Your commit message is missing either a JIRA Issue ('AG-XXXX' or 'ND-XXXX') or one of allowed prefixes.
